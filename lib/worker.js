...
Worker.prototype.start = function () {
    ...
    connection.getNativeClient()
        .then(function (result) {
            ...
            return self.loadLastResumeToken(db);
        })
        .then(function (resumeToken) {
...
Worker.prototype.loadLastResumeToken = function (db, callback) {
    ...
    // MongoDB driver v6.x returns Promises, not callbacks
    var promise = resumeTokenCollection.findOne({ workerId: this.workerId })
        .then(function (doc) {
    ...
    if (typeof callback === 'function') {
        promise.then(function (resumeToken) {
...
Worker.prototype.validateResumeToken = function (token) {
    ...
    // Check if it's a serialized BSON object with _data property (hex-encoded buffer)
    // This can happen when BSON objects are serialized via JSON.stringify()
    if (token._data && typeof token._data === 'string') {
        return true;
    }
...
Worker.prototype.saveResumeToken = function (db, resumeToken, callback) {
    ...
    if (!this.validateResumeToken(resumeToken)) {
        var error = new Error('Invalid resume token structure: ' + JSON.stringify(resumeToken));
    ...
    // Otherwise, debounce to batch saves OR return Promise for immediate save
    if (typeof callback === 'function') {
        ...
        // Return Promise for immediate save when no callback provided
...
Worker.prototype.queueResumeTokenSave = function (resumeToken, callback) {
    ...
    if (!this.validateResumeToken(resumeToken)) {
        ...
        if (typeof callback === 'function') callback(new Error('Invalid resume token structure'));
        return;
    }
...
Worker.prototype.updateResumeTokenTimestamp = function (callback) {
    ...
    var promise = resumeTokenCollection.updateOne(
        ...
        if (result.matchedCount === 0 && result.upsertedCount === 0) {
            throw new Error('Failed to update resume token timestamp');
        }
        return;
    });
    ...
    changeStream.on('change', function (change) {
        ...
        if (change.operationType === 'update') {
            ...
            if (!jobDoc && change.documentKey && change.documentKey._id) {
                ...
                collection.findOne({ _id: change.documentKey._id }, findOptions)
                    .then(function (doc) {
                        ...
                        // Check if status was actually updated to QUEUED (transition from any status to QUEUED)
                        var statusChangedToQueued = false;
                        if (change.updateDescription && change.updateDescription.updatedFields) {
                            // Check if the status field was updated and the new value is QUEUED
                            // This detects transitions from any status (FAILED, COMPLETE, CANCELLED, DEQUEUED) to QUEUED
                            statusChangedToQueued = change.updateDescription.updatedFields.hasOwnProperty('status') &&
                                change.updateDescription.updatedFields.status === Job.QUEUED;
                        } else {
                            // If we can't determine from updateDescription, assume status was updated to QUEUED
                            // (safer to process than to skip - handles edge cases where updateDescription is missing)
                            statusChangedToQueued = true;
                        }

                        if (statusChangedToQueued) {
                            // Process the job
                    ...
                    .catch(function (err) {
                        // Check if worker is still working before processing error
                        if (!self.working) {
                            return;
                        }
                        self.emit('error', err);
                    });
                return; // Exit early, will process in promise
            }
            ...
            if (jobDoc && jobDoc.status === Job.QUEUED) {
                ...
                if (change.updateDescription && change.updateDescription.updatedFields) {
                    // Check if the status field was updated and the new value is QUEUED
                    // This ensures we only process when status actually changed to QUEUED, not just lookups
                    statusChangedToQueued = change.updateDescription.updatedFields.hasOwnProperty('status') &&
                        change.updateDescription.updatedFields.status === Job.QUEUED;
                } else {
                    // If we can't determine from updateDescription, assume status was updated to QUEUED
                ...
                // Process if status changed to QUEUED or if we can't determine
                if (statusChangedToQueued) {
                    // Check if we've recently processed this job (deduplication)
                    var jobId = jobDoc._id ? jobDoc._id.toString() : null;
                    if (jobId && self.recentlyProcessedJobs.has(jobId)) {
...
Worker.prototype.startChangeStreamHealthMonitoring = function () {
    ...
    this.changeStreamHealthCheckInterval = setInterval(function () {
        ...
        if (self.lastResumeToken && self.db && timeSinceLastTokenUpdate > sixDaysInMs && !self.updatingResumeTokenTimestamp) {
            ...
            self.updateResumeTokenTimestamp()
                .then(function () {
...
/**
* Finds the index of the highest-priority pending change stream job
* Tie-breaker: lower _id first (matches dequeue sort order)
* @returns {Number}
*/
Worker.prototype.findHighestPriorityPendingIndex = function () {
    var bestIndex = 0;
    var bestPriority = -Infinity;

    for (var i = 0; i < this.pendingJobs.length; i++) {
        var priority = this.pendingJobs[i].jobDocument.priority || 0;
        ...
        if (priority === bestPriority) {
            var bestId = this.pendingJobs[bestIndex].jobDocument._id;
            var currentId = this.pendingJobs[i].jobDocument._id;
            if (bestId && currentId && currentId.toString() < bestId.toString()) {
                bestIndex = i;
            }
        }
    }
...
/**
* Removes a pending job entry and returns its resume token when IDs match
* @param {String} jobId
* @param {Object} [fallbackToken]
* @returns {Object|null}
*/
Worker.prototype.takePendingResumeToken = function (jobId, fallbackToken) {
    if (!jobId) {
        return fallbackToken || null;
    }

    for (var i = 0; i < this.pendingJobs.length; i++) {
        var pendingId = this.pendingJobs[i].jobDocument._id ?
            this.pendingJobs[i].jobDocument._id.toString() : null;

        if (pendingId === jobId) {
            var token = this.pendingJobs[i].resumeToken || fallbackToken || null;
...
Worker.prototype.processJobInternal = function (queue, jobDocument, resumeToken) {
    ...
    // Always claim highest-priority queued job first, not just the change stream event job
    queue.claimNextJob({
        ...
        .then(function (job) {
            ...
            job._resumeToken = self.takePendingResumeToken(jobId, resumeToken);
...
* Always prefers highest-priority queued jobs before pending change stream events
*/
Worker.prototype.processNextPendingJob = function () {
    ...
    this.dequeue()
        .then(function (job) {
            if (job) {
                var jobId = job.data._id ? job.data._id.toString() : null;
                job._resumeToken = self.takePendingResumeToken(jobId);

                var queuePercent = (self.pendingJobs.length / self.maxPendingJobs) * 100;
                if (queuePercent < 50 && self.changeStreamPaused) {
                    self.resumeChangeStream();
                }
            ...
            if (self.pendingJobs.length === 0) {
                self.processing = false;
                return;
            }

            var bestIndex = self.findHighestPriorityPendingIndex();
            var next = self.pendingJobs.splice(bestIndex, 1)[0];
            if (!next) {
                self.processing = false;
                return;
            }
            ...
            if (nextJobId) {
                self.pendingJobIds.delete(nextJobId);
            }
            ...
            self.processJobInternal(next.queue, next.jobDocument, next.resumeToken);
        })
        .catch(function (err) {
            self.processing = false;
            self.emit('error', err);
...
Worker.prototype.dequeue = function (callback) {
    ...
    function tryNextQueue() {
        ...
        return queue.dequeue({ minPriority: self.minPriority, callbacks: self.callbacks })
            .then(function (job) {
                if (job) {
                    return job;
                }
...
Worker.prototype.error = function (job, err, callback) {
    ...
    if (remaining > 0) {
        ...
        promise = job.delay(wait);
    } else {
        promise = job.fail(err);
    }
