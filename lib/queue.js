...
/**
* Claims the next available job, preferring higher priority jobs first.
* Uses priority-sorted dequeue first, then falls back to claiming a specific
* change stream job when no higher-priority work is available.
* @param {Object} options - dequeue options (minPriority, callbacks)
* @param {Object} [jobDocument] - Optional job document from a change stream event
* @param {Function} [callback] - callback with (err, job)
*/
Queue.prototype.claimNextJob = function (options, jobDocument, callback) {
    var self = this;

    if (typeof jobDocument === 'function') {
        callback = jobDocument;
        jobDocument = null;
    }
    ...
    var promise = this.dequeue(options).then(function (job) {
        if (job || !jobDocument) {
            return job;
        }

        return self.dequeueFromChangeStream(jobDocument, options);
    });
